...
public interface ProcessedRequestsDao extends JpaRepository<ProcessedRequests, Integer>{
	...
	@Transactional
	@Modifying
	@Query(value = "UPDATE processed_requests SET request_status=2 WHERE request_id = ?1 ", nativeQuery = true)
	public void updatedPStatus(Integer requestId);
	
	//fetches all the the requests approved by pm
	@Query(value="SELECT * FROM processed_requests WHERE request_status = ?1", nativeQuery = true)
